"""Experiment 9: Title: Final Model Training & Physics-Based Interpretability

Purpose:
To train the "Production" model using the optimal configuration identified in previous experiments (Window=45m, Top-25 Coefficients) and, crucially, to decode the model's internal logic into human-readable Solar Physics terms.
...
Semantic Mapping: It reverse-engineers the random forest's internal decision trees. It maps abstract "Interval Indices" back to physical Time Slices (e.g., "T-45m to T-0m") and Spectral Features (e.g., "Proton Flux Magnitude").
...
def main():
    ...
    output_dir = data_root / "reports" / "experiment9"
    output_dir.mkdir(parents=True, exist_ok=True)
    stamp = datetime.now().strftime("%Y%m%d_%H%M%S")

    model_path = output_dir / f"experiment9_tsf_model_{stamp}.joblib"
    ...
    model_info_path = output_dir / f"experiment9_tsf_model_metadata_{stamp}.json"
    ...
    importance_table_path = output_dir / f"experiment9_feature_importance_table_{stamp}.csv"
    importance_df.to_csv(importance_table_path, index=False)
    print(f"Saved feature importance table: {importance_table_path}")

    fi_path = output_dir / f"experiment9_feature_importance_{stamp}.png"
    ...
    if not tsf_semantic_df.empty:
        ...
        tsf_semantic_table_path = output_dir / f"experiment9_feature_importance_tsf_semantic_table_{stamp}.csv"
        ...
        tsf_semantic_plot_path = output_dir / f"experiment9_feature_importance_tsf_semantic_{stamp}.png"
        ...
        tsf_timeline_plot_path = output_dir / f"experiment9_feature_importance_tsf_timeline_{stamp}.png"
