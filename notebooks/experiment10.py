"""Experiment 10: Run RandomForest on FFT magnitude+phase features using an
...
def main():
    ...
    output_dir = data_root / "reports" / "experiment10"
    output_dir.mkdir(parents=True, exist_ok=True)
    stamp = datetime.now().strftime("%Y%m%d_%H%M%S")

    results_path = output_dir / f"experiment10_results_{stamp}.csv"
    results_df.to_csv(results_path, index=False)
    print(f"\nSaved results: {results_path}")

    val_heatmap_path = output_dir / f"experiment10_val_metrics_heatmap_{stamp}.png"
    plot_metric_heatmaps(results_df, val_heatmap_path, split_prefix="val")
    print(f"Saved validation plot: {val_heatmap_path}")

    test_heatmap_path = output_dir / f"experiment10_test_metrics_heatmap_{stamp}.png"
    ...
    best_path = output_dir / f"experiment10_best_config_{stamp}.csv"
