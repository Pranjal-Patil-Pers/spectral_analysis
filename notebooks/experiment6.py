"""Experiment 6: Run TimeSeriesForest on FFT magnitude+phase features using an
...
def build_fft_features_all(
    ...
    Output format for aeon TSF: (N, C_fft, T_fft)
    ...
    for fft_window_size in fft_window_sizes:
        for lag in forecast_lags:
            ...
            for coeff in coeff_options:
                ...
                y_pred_train = model.predict(X_train_spec)
                y_pred_val = model.predict(X_val_spec)
                y_pred_test = model.predict(X_test_spec)
...
def main():
    ...
    output_dir = data_root / "reports" / "experiment6"
    output_dir.mkdir(parents=True, exist_ok=True)
    stamp = datetime.now().strftime("%Y%m%d_%H%M%S")

    results_path = output_dir / f"experiment6_results_{stamp}.csv"
    results_df.to_csv(results_path, index=False)
    print(f"\nSaved results: {results_path}")

    val_heatmap_path = output_dir / f"experiment6_val_metrics_heatmap_{stamp}.png"
    plot_metric_heatmaps(results_df, val_heatmap_path, split_prefix="val")
    print(f"Saved validation plot: {val_heatmap_path}")

    test_heatmap_path = output_dir / f"experiment6_test_metrics_heatmap_{stamp}.png"
    ...
    best_path = output_dir / f"experiment6_best_config_{stamp}.csv"
    best.to_frame().T.to_csv(best_path, index=False)

    print("\n" + "=" * 70)
    print(
        "Best configuration (sorted by val_css, test_css, val_tss, test_tss, "
        "val_hss, test_hss, then F1 ties):"
    )
    for col in [
        "window_size",
        "forecast_lag_min",
