...
def enforce_channel_constraints(
    sample: np.ndarray,
    channel_names: list[str],
    channel_ranges: dict[str, tuple[float, float]],
    channel_order_constraints: list[tuple[str, str]] | None = None,
) -> None:
    """Validate per-channel ranges and inter-channel ordering constraints.

    Raises ValueError when any constraint is violated.
    """
    arr = np.asarray(sample, dtype=np.float64)

    for channel_name, (lo, hi) in channel_ranges.items():
        if channel_name not in channel_names:
            raise ValueError(f"Missing required channel '{channel_name}' in channel_names.")
        idx = int(channel_names.index(channel_name))
        values = arr[:, idx]
        if np.any(values <= lo) or np.any(values >= hi):
            raise ValueError(
                f"Channel '{channel_name}' violates range constraint: "
                f"expected {lo} < value < {hi}."
            )

    if channel_order_constraints:
        for higher_name, lower_name in channel_order_constraints:
            if higher_name not in channel_names:
                raise ValueError(f"Missing required channel '{higher_name}' in channel_names.")
            if lower_name not in channel_names:
                raise ValueError(f"Missing required channel '{lower_name}' in channel_names.")

            higher_idx = int(channel_names.index(higher_name))
            lower_idx = int(channel_names.index(lower_name))
            if np.any(arr[:, higher_idx] >= arr[:, lower_idx]):
                raise ValueError(
                    f"Inter-channel constraint violated: expected {higher_name} < {lower_name} "
                    f"at every timestep."
                )


def validate_flux_constraints(sample: np.ndarray, channel_names: list[str]) -> None:
    enforce_channel_constraints(
        sample=sample,
        channel_names=channel_names,
        channel_ranges={
            "p3_flux_ic": (1e-3, 1e2),
            "p5_flux_ic": (1e-3, 1e1),
            "p7_flux_ic": (1e-3, 1e-1),
        },
        channel_order_constraints=[("p7_flux_ic", "p5_flux_ic"), ("p5_flux_ic", "p3_flux_ic")],
    )


def generate_valid_counterfactual(
    dice,
    ...
    """Retry DiCE until a reconstructed counterfactual passes all flux constraints."""
    last_error = ""
    ...
    for attempt in range(max_attempts):
        ...
        try:
            ...
            validate_flux_constraints(cf_recon, channel_names)
            return cf_recon, cf_prediction, True, ""
        except Exception as exc:
            last_error = f"attempt {attempt + 1}/{max_attempts}: {exc}"

    return np.empty((0, 0), dtype=np.float32), -1, False, last_error
...
def run_variant(
    ...
    results_path = out_dir / f"experiment18_{variant_name}_results_{variant_stamp}.csv"
    selection_path = out_dir / f"experiment18_{variant_name}_selection_{variant_stamp}.csv"
    out_dir.mkdir(parents=True, exist_ok=True)
    ...
    saved_model_paths = save_model_artifacts_v17(
        ...
        prefix=f"experiment18_{variant_name}",
    )

    val_plot_by_lag_path = out_dir / f"experiment18_{variant_name}_val_css_by_lag_{variant_stamp}.png"
    plot_toppercent_performance_by_lag(results_df, val_plot_by_lag_path, metric_col="val_css")

    test_plot_by_lag_path = out_dir / f"experiment18_{variant_name}_test_css_by_lag_{variant_stamp}.png"
    plot_toppercent_performance_by_lag(results_df, test_plot_by_lag_path, metric_col="test_css")

    val_plot_by_window_path = out_dir / f"experiment18_{variant_name}_val_css_by_window_{variant_stamp}.png"
    plot_toppercent_performance_by_window(results_df, val_plot_by_window_path, metric_col="val_css")

    test_plot_by_window_path = out_dir / f"experiment18_{variant_name}_test_css_by_window_{variant_stamp}.png"
    plot_toppercent_performance_by_window(results_df, test_plot_by_window_path, metric_col="test_css")

    feature_count_plot_path = out_dir / f"experiment18_{variant_name}_features_required_cumimp_gt50_{variant_stamp}.png"
    ...
    best_summary_path = out_dir / f"experiment18_{variant_name}_best_model_summary_{variant_stamp}.csv"
...
def generate_transform_counterfactual_reports(
    ...
    for example in chosen_examples:
        ...
        try:
            cf_recon, cf_prediction, cf_generated, error_message = generate_valid_counterfactual(
                dice=dice,
                query_df=query_df,
                feature_names=feature_names,
            ...
            combined_plot_path = method_dir / "combined" / (
                f"experiment18_{method_name}_cf_sample_{sample_id}_lag{lag}_w{fft_window_size}_"
...
def main():
    ...
    output_root = data_root / "reports" / "experiment18"
    ...
    for method_name, spec in method_specs.items():
        print(f"Experiment 18:{method_name} variant starting...")
        ...
        counterfactual_summary_path = output_root / method_name / f"experiment18_{method_name}_counterfactual_summary_{run_stamp}.csv"
        ...
        gallery_path = output_root / method_name / f"experiment18_{method_name}_counterfactual_gallery_{run_stamp}.png"
