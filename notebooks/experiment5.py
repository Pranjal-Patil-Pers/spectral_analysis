...
# ==========================================
# G. SHAP EXPLAINABILITY (BEESWARM) — FINAL FIX
# ==========================================

trained_featurizer = pipeline.named_steps['spec']
trained_rf = pipeline.named_steps['rf']

X_test_spec = trained_featurizer.transform(X_test)
feature_names = trained_featurizer.feature_names_

explainer = shap.TreeExplainer(trained_rf)
shap_values = explainer.shap_values(X_test_spec)

print("Raw SHAP shape:", shap_values.shape)
print("Feature matrix shape:", X_test_spec.shape)
...
if isinstance(shap_values, list):
    ...
    shap_vals_to_plot = shap_values[:, :, 1]  # Positive (SEP) class
else:
    # Already 2D
    shap_vals_to_plot = shap_values

print("Final SHAP used for plot:", shap_vals_to_plot.shape)
...
plt.title("SHAP Beeswarm Plot – Positive SEP Class", fontsize=14)
